...
def main(path1):
    ...
    h=240
    led_gs = np.array(
        [[155.7, 32, h], [154.9, 152.2, h], [24.4, 151.2, h], [23.8, 92.4, h], [84.2, 91.9, h], [240, 0, h],
         [24.4, 151.2, h], [84.2, 152.3, h], [154.9, 152.2, h], [25.3, 211.9, h], [84.7, 272.5, h],
         [156.2, 211.8, h]])
    # img = cv2.imread(path1 + ".jpg")  # 读取图片
    img_match = cv2.imread(path1 + "C.jpg")
    # row = len(img)
    # col = len(img[0])
    # if col > row:
    #     flag1 = 1
    ...
    # ################### 输入图二 #######################
    num2 = 20231212225832
    path2 = "image/no/" + str(num2)
    num3 = 20231212225852
    path3 = "image/no/" + str(num3)
    num4 = 20231212225920
    path4 = "image/no/" + str(num4)
    img1_match = cv2.imread(path2 + "C.jpg")
    img2_match = cv2.imread(path3 + "C.jpg")
    img3_match = cv2.imread(path4 + "C.jpg")
    # img4 = cv2.imread( path2 + ".jpg")  # 读取图片
    # img4_match = cv2.imread(path2 + "C.jpg")
    # ################### 图像处理 #######################
    # [position_x, position_y] = VO_position.Ellipse(id_list1, contours_list1, flag1)
    indexid1 = 0
    indexid2 = 0
    end = time.perf_counter()
    print("第二帧解码用时：", end - start, "秒")
    dis1= match.how_match(img_match, img1_match)
    dis2 = match.how_match(img_match, img2_match)
    dis3 = match.how_match(img_match, img3_match)
    pos = dis1 * led_gs[0] + dis2 * led_gs[1] + dis3 * led_gs[2]
    pos=pos/(dis1+dis2+dis3)
    print(pos)
    # if len(id_list) == 0:
