...
def Vo_Frames(id_list1, contours1, centerlist1, flag1, id_list2, contours2, centerlist2, flag2, R):
    ...
    if dist[0]>dist[1]:
        # idfinal = id_sure[1]
        idfinal = 1
        led_nv_rs_estfinal = led_nv_rs_est[1][:, 1]
    else:
        # idfinal = id_sure[1]
        idfinal = 0
        led_nv_rs_estfinal = led_nv_rs_est[1][:, 0]
    led_nv_rs_estfinal = qp_nv_final
...
def Vo_Ellipse(id_list, contours, centerlist, flag, mnum):
    h = 240  # 实验平台高度
    ...
    led_gs = np.array(
        [[90, 30, h], [85.7, 33.2, h], [155.7, 32, h], [23.8, 92.4, h], [84.2, 91.9, h], [240, 0, h],
    ...
    for i in range(1):
        ...
        if temp_sign[2] != temp_sign[0] and temp_sign[2] != temp_sign[1]:
            ...
            if B == 1:  # XOZ投影
                ...
                led_Gc_trans = np.array([[crspt[0][0], crspt[0][1]], [0, 0], [crspt[1][0], crspt[1][1]]])
                led_nv_trans = np.array([[k_cut[0], k_cut[1]], [0, 0], [-1, -1]])
                n = 1
            elif B == 0:  # YOZ投影
                #led_Gc_trans = np.array([np.zeros((1, 2),dtype=int), crspt[0,:],  crspt[1,:]])
                led_Gc_trans = np.array([[0, 0], [crspt[0][0], crspt[0][1]], [crspt[1][0], crspt[1][1]]])
        ...
        if temp_sign[0] != temp_sign[2] and temp_sign[0] != temp_sign[1]:
            ...
            if B == 2:  # XOZ投影
               # led_Gc_trans = np.array([crspt[1,:], crspt[0,:],np.zeros((1, 2),dtype=int)])
                led_Gc_trans = np.array([ [crspt[1][0], crspt[1][1]], [crspt[0][0], crspt[0][1]],[0, 0]])
        ...
        if temp_sign[1] != temp_sign[0] and temp_sign[1] != temp_sign[2]:
            ...
            if B == 1:  # XOZ投影
                ...
                led_Gc_trans = np.array([[crspt[0][0], crspt[0][1]], [crspt[1][0], crspt[1][1]], [0, 0]])
                led_nv_trans = np.array([[k_cut[0], k_cut[1]], [-1, -1], [0, 0]])
                n = 5
            elif B == -1:  # YOZ投影
                #led_Gc_trans = np.array([np.zeros((1, 2),dtype=int), crspt[1,:], crspt[0,:]])
                led_Gc_trans = np.array([[0, 0], [crspt[1][0], crspt[1][1]],[crspt[0][0], crspt[0][1]] ])
    ...
    for id in range(2):
        ...
        irr_judge = np.dot(led_nv_rs_est[:, id], -ledmk[:, interval[0]]) / np.linalg.norm(ledmk[:, interval[0]]) / np.linalg.norm(led_nv_rs_est[:, id])
        ...
        if n==1:
            co = b_led[id]/ (led_img_trans[2, interval]/ led_img_trans[0, interval] - k_cut[id]) #size: 1×length(interval)
            xa = [1]
            ya = led_img_trans[1, interval] / led_img_trans[0, interval]  #size: 1×length(interval)
            za = led_img_trans[2, interval] / led_img_trans[0, interval]  # size: 1×length(interval)
        elif n==2:
            co = b_led[id]/ (led_img_trans[2, interval]/ led_img_trans[1, interval] - k_cut[id])
            xa = led_img_trans[0, interval] / led_img_trans[1, interval]
            ya = [1]
            za = led_img_trans[2, interval] / led_img_trans[1, interval]
        elif n==3:
            co = b_led[id]/ (led_img_trans[1, interval] / led_img_trans[0, interval] - k_cut[id])
            xa = [1]
            ya = led_img_trans[1, interval] / led_img_trans[0, interval]
            za = led_img_trans[2, interval] / led_img_trans[0, interval]
        elif n==4:
            co = b_led[id] / (1-np.dot(k_cut[id], (led_img_trans[2, interval] / led_img_trans[0, interval])))
            xa = [1]
            ya = led_img_trans[1, interval] / led_img_trans[0, interval]
            za = led_img_trans[2, interval] / led_img_trans[0, interval]
        elif n==5:
            co = b_led[id] / (np.dot((1 - k_cut[id]), led_img_trans[1, interval]) / led_img_trans[0, interval])
            xa = [1]
            ya = led_img_trans[1, interval] / led_img_trans[0, interval]
            za = led_img_trans[2, interval] / led_img_trans[0, interval]
        elif n==5:
            co = b_led[id] / (np.dot((1 - k_cut[id]), led_img_trans[2, interval]) / led_img_trans[0, interval])
            xa = led_img_trans[0, interval] / led_img_trans[1, interval]
            ya = [1]
            za = led_img_trans[2, interval] / led_img_trans[1, interval]
        ledmk_trans_est = np.dot(co[0],[xa,ya,za]) # 图像上取点，LED上与之对应的点在ACS的坐标值
        led_mark_rs_est.append(np.dot(rot_mat_trans2rs, ledmk_trans_est))  #图像上取点，LED上与之对应的点在CCS的坐标值
        led_mark_rs_est[id] = np.dot(np.sign(led_mark_rs_est[id][2,0]), led_mark_rs_est[id])
...
def Ellipse(id_list, contours, flag):
    ...
    led_gs = np.array(
        [[24.3, 31.5, h], [85.7, 33.2, h], [155.7, 32, h], [23.8, 92.4, h], [84.2, 91.9, h], [155.2, 91.8, h],
         ...
         [156.2, 211.8, h]]).T  # LED的物理坐标
    led_gs = np.array(
        [[150, 150, h], [85.7, 33.2, h], [155.7, 32, h], [23.8, 92.4, h], [84.2, 91.9, h], [155.2, 91.8, h],
         [24.4, 151.2, h], [84.2, 152.3, h], [154.9, 152.2, h], [25.3, 211.9, h], [84.7, 272.5, h],
         [156.2, 211.8, h]]).T  # LED的物理坐标
