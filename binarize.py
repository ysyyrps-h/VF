...
def main(id_list,contours_list,center_list,point_xy, path):
    ...
    # ################### 输入图二 #######################
    img1 = cv2.imread("image/VLP1/" + path + ".jpg")  # 读取图片
    img1_match = cv2.imread("image/VLP1/" + path + "C.jpg")
    row = len(img1)
    col = len(img1[0])
    if col > row:
        flag1 = 1
...
id_list=data["arr_0"]
contours_list=data["arr_1"]
center_list=data["arr_2"]
id_listn=data["arr_0"]
contours_list=data["arr_1"]
...
center_list=[]
center_list.append(center_listn[0][0])
center_list.append(center_listn[0][1])
point_xyn=data["arr_3"]
point_xy=[]
point_xy.append(point_xyn[0])
point_xy.append(point_xyn[0])
pointsdata=np.load("points1/pointsdata.npz")
des1=pointsdata["arr_0"]
for i in range(100):
    num1 =20230309125137+ 2 * i
    path1 = str(num1)
    pos1, time1 = main(id_list,contours_list,center_list, point_xy,path1)
    pos = "new.txt"
